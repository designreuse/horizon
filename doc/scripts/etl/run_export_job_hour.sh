...
echo ${shour}
echo ${ehour}
echo "$BASEDIR/kitchen.sh -rep:DW_ETL -dir:/sqp_exec -job:jb_exec_export_load_job_by_s
ql_hour -level:Debug -log:${KETTLE_LOG_DIR}/etl.log.${etl_system}_${etl_job}.${today} 
${sdate} ${edate} ${shour} ${ehour} ${timezone} ${etl_system}_${etl_job}"
...
echo "/home/dwetl/kettle/data-integration/kitchen.sh -rep:DW_ETL -dir:/sqp_exec -job:j
b_exec_export_load_job_by_sql_hour -level:Debug -log:${KETTLE_LOG_DIR}/etl.log.${etl_system}_${etl_job}.${today} ${sdate} ${edate} ${shour} ${ehour} ${timezone} ${etl_system}_${etl_job}" >> $LOGFILE
$BASEDIR/kitchen.sh -rep:DW_ETL -dir:/sqp_exec -job:jb_exec_export_load_job_by_sql_hour -level:Debug -log:${KETTLE_LOG_DIR}/etl.log.${etl_system}_${etl_job}.${today} ${sdate} ${edate} ${shour} ${ehour} ${timezone} ${etl_system} ${etl_job} |tee ${KETTLE_LOG_DIR}/tmp.etl.log.${etl_system}_${etl_job}
