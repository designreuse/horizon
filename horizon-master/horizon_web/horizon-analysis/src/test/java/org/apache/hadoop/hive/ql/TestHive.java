...
import data.horizon.analysis.sql.HiveVisitor;
import data.horizon.analysis.sql.Visitor;
...
public class TestHive {
  ...
  static final private Log LOG = LogFactory.getLog(TestHive.class.getName());
  ...
  public void testInsertSelect() throws Exception {
    try {
      ReturnInfo rc = parseAndAnalyze("Create table t1 as select s.a, s.b from (select * from default.U union all select * from default.U) s", "testInsertSelect");

      LOG.info(explain((SemanticAnalyzer)rc.sem, rc.plan, rc.ast.dump()));

      analysisTableAccessInfo(rc.ast);
  ...
  private void analysisTableAccessInfo(ASTNode ast) {
    Visitor visitor = new HiveVisitor();

    visitor.visit(ast);

    // 遍历输出对表的操作信息
    visitor.getTableAccessInfo().forEach((key, value) -> {
