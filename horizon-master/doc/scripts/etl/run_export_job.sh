...
$BASEDIR/kitchen.sh -rep:DW_ETL -dir:/sqp_exec -job:jb_exec_export_load_job_by_sql -level:Debug -log:${KETTLE_LOG_DIR}/etl.log.${etl_system}_${etl_job}.${today} ${sdate} ${edate} ${etl_system} ${etl_job} ${timezone} |tee ${KETTLE_LOG_DIR}/tmp.etl.log.${etl_system}_${etl_job} 
